...
import { IntakeBrief, KnowledgeUnit } from '@/types';
...
export default function Brief({ data }: { data: IntakeBrief }) {
  const { source, knowledge_units, stats } = data;
    ...
    <div>
      ...
      <div style={{
        ...
        <div style={{
          ...
          background: 'linear-gradient(90deg, #00e5a0, #6c5ce7, transparent)',
        }} />
        ...
        <h2 style={{ fontSize: 20, fontWeight: 700, fontFamily: "'Syne', sans-serif", marginBottom: 12, lineHeight: 1.3 }}>
          {source.title}
        </h2>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' as const }}>
          <span style={{
            fontSize: 10, padding: '4px 10px', borderRadius: 6,
            background: '#1e2340', color: '#00e5a0',
            fontFamily: "'JetBrains Mono', monospace",
          }}>{SOURCE_LABELS[source.type] || source.type}</span>
          {source.author && <span style={{ fontSize: 13, color: '#8b90a8' }}>{source.author}</span>}
          {source.url && (
            <a href={source.url} target="_blank" rel="noopener noreferrer"
              style={{ fontSize: 12, color: '#4e5370', textDecoration: 'underline' }}>View source →</a>
          )}
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10, marginBottom: 32 }}>
        {[
          ...
          { value: stats.total_sources, label: 'Sources', color: '#f5a623' },
        ].map((s) => (
          ...
          {orderedTypes.map(([type, units]) => {
            ...
            return (
              <div key={type} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                ...
                <span style={{ fontSize: 11, color: '#4e5370', fontFamily: "'JetBrains Mono', monospace" }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column' as const, gap: 8 }}>
            {units.map((unit) => (
              <div key={unit.id} style={{
                ...
                {unit.context && (
                  <p style={{ fontSize: 12, color: '#4e5370', marginTop: 8, lineHeight: 1.5 }}>{unit.context}</p>
                )}
